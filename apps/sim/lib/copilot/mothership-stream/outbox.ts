...
const logger = createLogger('MothershipStreamOutbox')
...
export async function resetOutbox(streamId: string): Promise<void> {
...
export async function readEnvelopes(
  ...
  if (!Number.isFinite(afterSeq)) {
    logger.warn('Invalid non-numeric cursor in readEnvelopes, returning empty', {
      streamId,
      afterCursor,
    })
    return []
